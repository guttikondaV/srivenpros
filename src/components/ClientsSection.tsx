        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {clients.map((c, i) => (
            <motion.div
              ...
              className="flex flex-col items-center justify-center gap-3 rounded-xl bg-card border border-border/60 py-6 px-4 shadow-card hover:shadow-elevated hover:border-accent/30 transition-all"
            >
              <img
                src={c.logo}
                alt={`${c.name} logo`}
                ...
                loading="lazy"
              />
              <span className="text-xs font-medium text-muted-foreground tracking-wide text-center">
                {c.name}
              </span>
            </motion.div>
          ))}
